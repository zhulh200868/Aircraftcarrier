...
if is_backfile:
    Rthandler = RotatingFileHandler('%s'%logname,maxBytes=1*1024*1024,backupCount=10)
    Rthandler.setLevel(logging.INFO)
    # formatter = logging.Formatter('%(name)-12s: %(levelname)-8s %(message)s')
    formatter = logging.Formatter('[%(asctime)s] [%(filename)s] [%(module)s] [%(funcName)s] [line:%(lineno)d] [%(levelname)s] [%(threadName)s] %(message)s')
...
if is_writefile:
    ...
    '''
    logfile =  TimedRotatingFileHandler("%s"%logname, "D", 1, 5)
